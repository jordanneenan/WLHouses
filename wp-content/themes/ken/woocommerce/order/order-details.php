...
<table class="shop_table order_details">
	...
	<?php
				...
				if ( $has_refund && $key === 'order_total' ) {
					...
					if ( wc_tax_enabled() && 'incl' == $order->tax_display_cart ) {
						...
						if ( ! empty( $tax_del_array ) ) {
							$refunded_tax_del .= ' ' . sprintf( __( '(Includes %s)', 'mk_framework' ), implode( ', ', $tax_del_array ) );
						}

						if ( ! empty( $tax_ins_array ) ) {
							$refunded_tax_ins .= ' ' . sprintf( __( '(Includes %s)', 'mk_framework' ), implode( ', ', $tax_ins_array ) );
						}
					}
		...
		if ( $has_refund ) { ?>
			<tr>
				<th scope="row"><?php _e( 'Refunded:', 'mk_framework' ); ?></th>
		...
		if ( '' != $order->customer_note ) { ?>
			<tr>
				<th scope="row"><?php _e( 'Note:', 'mk_framework' ); ?></th>
...
<?php
	if ( $order->billing_email ) {
		echo '<tr><th>' . __( 'Email:', 'mk_framework' ) . '</th><td data-title="' . __( 'Email', 'mk_framework' ) . '">' . $order->billing_email . '</td></tr>';
	}

	if ( $order->billing_phone ) {
		echo '<tr><th>' . __( 'Telephone:', 'mk_framework' ) . '</th><td data-title="' . __( 'Telephone', 'mk_framework' ) . '">' . $order->billing_phone . '</td></tr>';
	}
